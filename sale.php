...
//검색시작
	...
	if(isset($searchColumn) && isset($searchText)) {
		$searchSql = ' where in_out_cate = "매출" AND ' . $searchColumn . ' like "%' . $searchText . '%"';
	} else {
...
//검색 끝
	...
	$query ="SELECT * FROM wine_in_out ".$searchSql."ORDER BY idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
  ...
  <section id="container">
    ...
    <section id="main-content" style="min-height:800px;">
	  <section class="wrapper">
		      ...
		      <div class="input-group">
			    <div class="input-group-btn search-panel">
                  ...
                  <ul class="dropdown-menu" role="menu">
                    <li><a href="#in_out_m">담당자명</a></li>
                    <li><a href="#payment_state">지불방식</a></li>
                    <li><a href="#memo">비고</a></li>
                  </ul>
                </div>
		...
		<form onsubmit="return validate();" action="./api/order_delete.php" 
			  ...
			  <table class="table table-bordered table-hover table-striped">
                <thead class="cf" style='background-color: #BDBDBD'>
                  <tr>
					  ...
					  <th class="numeric">처리날짜</th>
					  <th class="numeric">처리창고</th>
					  <th class="numeric">거래처이름</th>
					  <th class="numeric">매장이름</th>
					  <th class="numeric">지불방식</th>
					  <th class="numeric">출력구분</th>
                ...
                <tbody>
				  <?if($result!= 0){while($row = $conn->DBF()){?>
                    <tr>
					  ...
					  <td data-title="주문코드"><a href="#"><?echo $row['order_code'];?></a></td>
					  <td data-title="구분"><a href="#"><?echo $row['in_out_cate'];?></a></td>
					  <td class="numeric" data-title="담당자이름"><?echo $row['in_out_m'];?></td>
					  <td class="numeric" data-title="처리날짜"><?echo $row['in_out_date'];?></td>
					  <td class="numeric" data-title="처리창고"><?echo $row['in_out_ware'];?></td>
					  <td class="numeric" data-title="거래처이름"><?echo $row['com_name'];?></td>
					  <td class="numeric" data-title="매장이름"><?echo $row['store_name'];?></td>
					  <td class="numeric" data-title="지불방식"><?echo $row['payment_state'];?></td>
					  <td class="numeric" data-title="출력구분"><?echo $row['in_out_flag'];?></td>
		...
		<div class="row" style="text-align:right">
          <div class="col-lg-12" style=""> 
		    <button type="button" class="btn btn-default" onclick="location.href='order_registration.php'">신규</button>
