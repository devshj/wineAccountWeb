<?
	...
	switch($sCase)
	{
		...
		//담당자
		case 3:
		$searchSql = ' where in_out_manager like "%' .$de_mana. '%" and de_sp_cate = "입금"';
		break;
		...
		// 날짜 + 담당자
		case 5:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and in_out_manager like"%' .$de_mana. '%" and de_sp_cate = "입금"';
		break;

		// 거래처 + 담당자
		case 6:
		$searchSql = ' where com_name like "%' .$de_com. '%" and de_sp_cate = "입금" and in_out_manager like "%' .$de_mana. '%" and de_sp_cate = "입금"';
		break;

		// 날짜 + 거래처 + 담당자
		case 7:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and com_name like "%' .$de_com. '%" and in_out_manager like "%' .$de_mana. '%" and de_sp_cate = "입금"';
		break;
	}
					...
					<div class="col-lg-12" style="" id="print">
						<section id="no-more-tables">
							<table class="table table-bordered table-hover table-striped">
								...
								?>
								<tr>
									<td data-title="선택"><input type="checkbox" id="chk_info[]" name="chk_info[]" value="<?echo $row['idx'];?>"></td>
									<td data-title="입금명"><a href="./depositDetail.php?dNo=<?echo $row['de_sp_code'];?>"><?echo $row['de_sp_name'];?></a></td>
									<td class="numeric" data-title="거래처"><?echo $row['com_name'];?></td>
									<td class="numeric" data-title="담당자"><?echo $row['in_out_manager'];?></td>
									<td class="numeric" data-title="매장명"><?echo $row['store_name'];?></td>
									<td class="numeric" data-title="입금 날짜"><?echo $row['de_sp_date'];?></td>
									<td class="nuemric" data-title="비고"><?echo $row['memo'];?></td>
								</tr>
							<? } ?>
							</tbody>
						</table>
