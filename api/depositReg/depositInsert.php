<?
  ...
  try{
    ...
    values('".$deTitle."','입금','".$date."','".$date_from."','".$manager."','".$company."','".$store."','1','".$memo."')";
    $conn->DBQ($sql);
    $conn->DBE();
    ...
    for($i=0; $i<count($product_code); $i++) {
      ...
      values('입금','".$date."','".$product_code[$i]."','".$product_name[$i]."','".$amount[$i]."','".$supply_price[$i]."','".$surtax[$i]."','".$discount[$i]."','".$total_price[$i]."')";
      $conn->DBQ($sql); 
      $conn->DBE();
    }
