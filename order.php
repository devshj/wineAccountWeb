...
//검색시작
	...
	if(isset($searchColumn) && isset($searchText)) {
		$searchSql = ' where order_cate = "주문" AND ' . $searchColumn . ' like "%' . $searchText . '%"';
	} else {
		$searchSql = 'where order_cate = "주문"';
	}
//검색 끝
                ...
                <tbody>
				  <?if($result!= 0){while($row = $conn->DBF()){?>
                    <tr>
					  ...
					  <td data-title="분류"><a href="#"><?echo $row['order_cate'];?></a></td>
					  <td data-title="거래처이름"><a href="#"><?echo $row['com_name'];?></a></td>
					  <td class="numeric" data-title="매장이름"><?echo $row['store_name'];?></td>
					  <td class="numeric" data-title="담당자이름"><?echo $row['m_name'];?></td>
					  ...
					  <td class="numeric" data-title="창고"><?echo $row['ware_code'];?></td>
					  <td class="numeric" data-title="출력구분"><?echo $row['order_flag'];?></td>
