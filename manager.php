...
//검색 끝
	...
	$query = "SELECT count(*) FROM wine_employee ".$searchSql;
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
	...
	$query ="SELECT * FROM wine_employee ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
  ...
  <section id="container">
    ...
    <section id="main-content" style="min-height:800px;">
	  <section class="wrapper">
        <h3><i class="fa fa-angle-right"></i><a href="<?=$_SERVER['PHP_SELF']?>">직원 관리</a></h3>	 
		<div class="row mt">
                ...
                <tbody>
				  <?if($result!= 0){while($row = $conn->DBF()){?>
                    <tr>
                      ...
                      <td data-title="사원명"><a href="manager_form.php?no=<?echo $row ['idx'];?>">
					  <?echo $row['emp_name'];?></a></td>
                      <td data-title="핸드폰번호"><a href="manager_form.php?no=<?echo $row ['idx'];?>">
					  <?echo $row['emp_phone'];?></a></td>
                      <td class="numeric" data-title="직급"><?echo $row['emp_job'];?></td>
                      <td class="numeric" data-title="부서"><?echo $row['emp_dept'];?></td>
                    </tr>
				<?}
				}else{$empty = "결과가 없습니다."?>
				<?}?>
